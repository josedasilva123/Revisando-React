...
const Main = () => {
    ...
    const handleSubmit = React.useCallback((event) => {
        ...
        async function submit(){
            try{
                ...
                const hasRepo = repositorios.find(repo => repo.name === repositorio);
    ...
    return (
        <div className={styles.container}>
            ...
            <form className={styles.form + (error ? ' ' + styles.error : '')} onSubmit={handleSubmit}>
                <input 
                type="text" 
            ...
            <ul className={styles.list}>
                {repositorios.map(repo => (
                    <li key={repo.name}>
                        <span>
                            <button className={styles.deleteButton} onClick={() => handleDelete(repo.name)}>
                                <FaTrash size={14} />
                            </button>  
                            {repo.name}
                        </span>
